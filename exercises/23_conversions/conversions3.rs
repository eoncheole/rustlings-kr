// 이 연습문제에서는 `FromStr`을 구현해서 문자열로 저장된 데이터를 구조화된
// 타입으로 변환할 거야. `From` 트레이트와 달리 `FromStr`을 통한 변환은
// 실패할 수 있어서 `Result`를 반환해. 추가로, `FromStr`을 구현하면 문자열의
// `parse` 메서드를 사용해서 해당 타입의 객체를 생성할 수 있어.
// 자세한 내용은 문서를 읽어봐:
