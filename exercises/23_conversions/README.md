...
가장 간단한 형태의 타입 변환은 타입 캐스트 표현식이야. 이진 연산자 `as`로 표시돼. 예를 들어, `println!("{}", 1 + 1.0);`은 `1`이 정수이고 `1.0`이 부동소수점이라서 컴파일되지 않아. 하지만 `println!("{}", 1 as f32 + 1.0)`은 컴파일돼. [`using_as`](using_as.rs) 연습문제에서 이걸 다뤄.
...
- `From`과 `Into`는 [`from_into`](from_into.rs)에서 다뤄
- `TryFrom`과 `TryInto`는 [`try_from_into`](try_from_into.rs)에서 다뤄
- `AsRef`와 `AsMut`는 [`as_ref_mut`](as_ref_mut.rs)에서 다뤄
