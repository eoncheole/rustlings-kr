...
가장 간단한 형태의 타입 변환은 타입 캐스트 표현식이야. 이진 연산자 `as`로 표시돼. 예를 들어, `println!("{}", 1 + 1.0);`은 `1`이 정수이고 `1.0`이 부동 소수점이라서 컴파일되지 않아. 하지만 `println!("{}", 1 as f32 + 1.0)`은 컴파일돼. [`conversions1`](conversions1.rs) 연습문제에서 이걸 다뤄.
